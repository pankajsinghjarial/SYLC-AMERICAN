...
$sentmail = mail( SITE_ADMIN_EMAIL, $subject, $message, $headers );
	if($sentmail){
		$message = 'You add a car to your selection with following details.<br />
					 ...
					 E-mail: '.$_POST['email'].'<br/>
					 Phone Number: '.$_POST['phone'];
		mail( $_POST['email'], "Thank you", $message, $headers );
		?>
...
$sentmail = mail( SITE_ADMIN_EMAIL, $subject, $message, $headers );
	if($sentmail){ 
		$message = 'Thank you for contacting us. Our specialist will contact you soon.';
		mail( $_POST['email'], "Thank you", $message, $headers );
		?>
       <script src="<?php echo DEFAULT_URL; ?>/js/jquery.min.js" type="text/javascript"></script>
       <script type="text/javascript">
...
if(isset($manufacturer)){
	...
	$searched = '<br>Votre recherche pour ';
	if(isset($manufacturer)){
		...
		$searched .= ' Marque: <span class="searched">'.urldecode($manufacturer).'</span>';
		
		$dataArray['manufacturer'] = $common->getIdByOptionName(2,urldecode($manufacturer)); 
		/*$where .= "&aspectFilter(0).aspectValueName=".urlencode($common->getOptionNameById($manufacturer));
	...
	if(isset($model) && $model != '' && $model != 'Not+Specified'){
		...
		$searched .= ', Modèle: <span class="searched">'.urldecode($model).'</span>';
...
if(isset($_POST) && $submit == 'sub_home'){
	if(isset($_POST['mileage']) && $_POST['mileage'] != ''){
		...
		$searched .= ', Kilométrage du véhicule: <span class="searched">'.urldecode($_POST['mileage']).'</span>';
		$aspect_count++;
	}
	if(isset($_POST['fuel']) && $_POST['fuel'] != ''){
		...
		$searched .= ', Type de carburant: <span class="searched">'.urldecode($_POST['fuel']).'</span>';
		$aspect_count++;
	}
	if(isset($_POST['bodyStyle']) && $_POST['bodyStyle'] != ''){
		...
		$searched .= ', Type de carrosserie: <span class="searched">'.urldecode($_POST['bodyStyle']).'</span>';
		$aspect_count++;
	}
	if(isset($_POST['extColor']) && $_POST['extColor'] != ''){
		...
		$searched .= ', Couleur extérieure: <span class="searched">'.urldecode($_POST['extColor']).'</span>';
		$aspect_count++;
	}
	if(isset($_POST['interColor']) && $_POST['interColor'] != ''){
		...
		$searched .= ', Couleur intérieure: <span class="searched">'.urldecode($_POST['interColor']).'</span>';
		$aspect_count++;
	}
...
if(isset($price) && $price != ''){
	...
	$searched .= " avec gamme de Prix <span class=\"searched\"> $".$price[0] * 1000 ." USD  to $".$price[1] * 1000 ." USD </span>";
...
$apicall .= "&outputSelector=aspectHistogramContainer.aspect['Model']";
}
